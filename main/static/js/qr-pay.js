...
(function (global) {
  ...
  async function tryCreateServerSession() {
    try {
  ...
  async function resolveScanUrl(sessionId) {
    let base = window.location.origin;
    try {
      const res = await fetch("/api/pay/base", { cache: "no-store" });
      const data = await res.json();
      if (data && data.base) base = String(data.base).replace(/\/$/, "");
    } catch (_) {}
    return `${base}/pay/scan/${encodeURIComponent(sessionId)}`;
  }
  ...
  function mount(opts) {
    ...
    function showPaid() {
      ...
      if (sessionId) {
        ...
        fetch(`/api/pay/confirm/${encodeURIComponent(sessionId)}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        }).catch(() => {});
      }
    ...
    async function pollServer() {
      ...
      try {
        const res = await fetch(
          `/api/pay/status/${encodeURIComponent(sessionId)}?t=${Date.now()}`,
          { cache: "no-store" }
        );
        const data = await res.json();
        if (data && data.paid) showPaid();
      } catch (_) {}
    }
