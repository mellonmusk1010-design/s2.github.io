...
(() => {
  ...
  /** Flask 경로(/static/coupang/...) → 정적 배포 상대경로로 보정 */
  function resolveProductImage(url) {
    if (!url) return "";
    if (url.startsWith("/static/coupang/")) {
      return "assets/coupang/" + url.slice("/static/coupang/".length);
    }
  ...
  async function loadCatalog() {
    ...
    try {
      ...
      for (const p of state.catalog.products) {
        p.image = resolveProductImage(p.image);
      }
