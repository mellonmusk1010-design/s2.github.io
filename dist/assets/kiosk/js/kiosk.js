...
(() => {
  ...
  function unitPriceFor(product, optionType) {
    // product.price 는 항상 단품 실습가. 세트는 setExtra 가산.
    const base = product.price || 0;
    const extra = state.menu.setDefaults?.setExtra || 2800;
    if (optionType === "set") return base + extra;
    return base;
  }
  ...
  async function loadMenu() {
    ...
    try {
      const res = await fetch("assets/data/mcdonalds_menu.json");
  ...
  function finishPractice() {
    window.location.href = "practice.html?done=kiosk";
  }
